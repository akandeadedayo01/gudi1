...
include("breadcrumbs/subcategory.php");
...
if(isset($_GET['sub']) && !empty($_GET['sub'])){
     $subID = escape($_GET['sub']);
}



//Subcategories
$subqry = "SELECT subcategories FROM subcategories WHERE id = '$subID'";
$subrst = $conn->query($subqry);
$sub = mysqli_fetch_array($subrst);
...
//Attribute Queries
$sattr = "SELECT attribute, id FROM products WHERE subID = '$subID'";
$attrst = $conn->query($sqry);
$attrow = mysqli_fetch_array($attrst);
$attr = $attrow['attribute'];
$attr = $attrow['id'];
...
?>
 ...
 <div class="container-fluid">
     <div class="row">
        <div class="col-md-12">
                <h2 class="text-center" style="margin-bottom:10px;"><?= $sub['subcategories']; ?></h2>
                 <?php while($prod=mysqli_fetch_array($srst)){?>
